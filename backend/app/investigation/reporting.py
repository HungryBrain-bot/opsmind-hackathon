...
class InvestigationReportBuilder:
    ...
    def to_markdown(self, result: InvestigationResult) -> str:
        ...
        lines = [
            ...
            "## Verdict",
            "",
            report.verdict or "No verdict generated.",
            "",
        ...
        for item in report.decision_trace.explanations:
            ...
            lines += [
                f"### {item.title} — {round(item.confidence * 100)}% [{marker}]",
                "",
                item.summary,
                "",
                item.why_selected or item.why_not_selected or "",
                "",
            ]
            if item.supporting_evidence:
                lines += ["**Supporting evidence**", ""]
                lines += [
                    f"- `{e.id}` — {e.title} ({e.source}, {e.reliability})"
        ...
        lines += [f"{i}. {a}" for i, a in enumerate(report.recommended_actions, 1)] or [
            "No automated recommendations were generated."
        ]
